...
1. **所有终端命令通过 tmux 执行** — 参见 `skills/tmux-multi-shell/SKILL.md`
...
3. **每次代码变更必须上板测试** — 参见 `skills/automated-testing/SKILL.md`
...
1. **启动 tmux 工作环境**（最先执行）
   ```bash
   # 幂等创建 — 参见 skills/tmux-multi-shell/SKILL.md
   ...
   # 定义 tmux_exec 函数（参见 skills/tmux-multi-shell/SKILL.md §3）
   ```
...
详细操作参见：
- **`skills/tmux-multi-shell/SKILL.md`** — tmux 会话管理、sentinel 命令执行模式、输出捕获
- **`skills/automated-testing/SKILL.md`** — 编译→烧录→串口→浏览器 完整测试链
