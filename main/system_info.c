...
static void add_chip_info(cJSON *root)
{
    ...
    esp_chip_info(&chip);
    cJSON *obj = cJSON_AddObjectToObject(root, "chip");
    cJSON_AddStringToObject(obj, "model", "ESP32");
    cJSON_AddNumberToObject(obj, "cores", chip.cores);
    cJSON_AddNumberToObject(obj, "revision", chip.revision);
    cJSON_AddBoolToObject(obj, "wifi", (chip.features & CHIP_FEATURE_WIFI_BGN) != 0);
    cJSON_AddBoolToObject(obj, "bt", (chip.features & CHIP_FEATURE_BT) != 0);
}

static void add_memory_info(cJSON *root)
{
...
static void add_health_info(cJSON *root)
{
